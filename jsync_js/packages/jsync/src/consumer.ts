...
import { ADD, cloneJson, decodeAndValidate, setOwn, SNAPSHOT } from './value.js';
    ...
    if (Array.isArray(current)) {
      if (typeof segment !== 'number') {
        throw new JsyncError(
          ...
          .withContext('while resolving an intermediate ADD path segment');
      }
      if (segment >= current.length) {
        throw new JsyncError(
          ...
          .withMetadata('segment_index', segmentIndex)
          .withContext('while resolving an ADD path parent');
      }
